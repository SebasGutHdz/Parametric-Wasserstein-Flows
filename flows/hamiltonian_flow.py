...
def compute_hamiltonian(
    ...
    potential_energy, _, _, _, _ = potential.evaluate_energy(
...
def run_hamiltonian_flow(
    ...
    for iteration in p_bar:
        ...
        if iteration == 0:
            _, samples0, _, _, _ = potential.evaluate_energy(
                current_parametric_model, test_data_set
            )
        ...
        momentum_norm = jnp.sqrt(
            sum(
                jax.tree.leaves(jax.tree.map(lambda x: jnp.sum(x**2), current_momentum))
            )
        )
        param_norm = jnp.sqrt(
            sum(jax.tree.leaves(jax.tree.map(lambda x: jnp.sum(x**2), current_params)))
        )
        param_increment = jnp.sqrt(
            sum(
                jax.tree.leaves(
                    jax.tree.map(
                        lambda a, b: jnp.sum((a - b) ** 2), current_params, prev_parms
                    )
                )
            )
        )
        ...
        if (
            ...
            current_energy, samples1, _, _, _ = potential.evaluate_energy(
                current_parametric_model, test_data_set, current_params
            )
            ...
            average_displacement = jnp.mean(
                jnp.linalg.norm(samples1 - samples0, axis=1)
            )
            ...
            iterations_so_far = (
                jnp.linspace(0, len(hamiltonian_history) - 1, len(hamiltonian_history))
                * h
            )
            ax_cons.plot(
                iterations_so_far, hamiltonian_history, "b-", label="Hamiltonian"
            )
            ax_cons.axhline(
                y=initial_hamiltonian,
                color="r",
                linestyle="--",
                alpha=0.5,
    ...
    return {
        ...
        "convergence_info": {
            ...
            "relative_drift": (
                total_drift / abs(initial_hamiltonian)
                if initial_hamiltonian != 0
                else float("inf")
            ),
